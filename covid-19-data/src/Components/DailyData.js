...
function makeDailyAndFixNegativeCases(data){
    let i;
    data[data.length-1].Cases = data[data.length-1].Cases - data[data.length-2].Cases
    for (i=data.length-2;i>0;i--){
        data[i].Cases = data[i].Cases - data[i-1].Cases
        if (data[i+1].Cases<0 && data[i].Cases>0 && -data[i+1].Cases <= data[i].Cases){
          data[i].Cases = data[i].Cases + data[i+1].Cases
          data[i+1].Cases = 0
        }
    }
...
class DailyData extends React.Component {
  ...
  componentDidMount() {
    const dailyData = makeDailyAndFixNegativeCases(this.props.dataWithoutDetails)
    this.setState({
        dailyData
    });
  }
